...
let currentStep = 0;
const otkPhotos = ["cover.jpg", "otk/1.jpg", "otk/2.jpg", "otk/3.jpg", "otk/4.jpg", "otk/5.jpg", "otk/6.jpg"];
const wishes = [
    "Happy Birthday Angelinka 🫶🏻",
    "I wish you all the very best!",
    "Health, happiness, and unforgettable moments",
...
function showBook() {
    ...
    rSide.addEventListener('click', () => {
        ...
        let frontImg = otkPhotos[currentStep * 2];
        let backImg = otkPhotos[currentStep * 2 + 1];
        let nextImg = otkPhotos[currentStep * 2 + 2];

        leaf.innerHTML = `
            <div class="leaf-side leaf-front"><img src="${frontImg}"></div>
            <div class="leaf-side leaf-back"><img src="${backImg}"></div>
        `;
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                if (nextImg) nextRImg.src = nextImg;
                if (currentStep === 0) lSide.style.background = "#fffafa";
                if (currentStep === 3) {
                    setTimeout(() => {
                        bookBox.style.transform = "translateX(-150vw) rotateZ(-10deg)";
...
function createHeartGallery() {
    ...
    const scale = window.innerWidth < 800 ? 12 : 18;
    ...
    for (let i = 1; i <= total; i++) {
        ...
        setTimeout(() => {
            img.style.opacity = "1";
            img.style.left = `${x * scale + centerX - 50}px`;
            img.style.top = `${y * scale + centerY - 65}px`;
